/**
 * Общие хелперы для генераторов R01–R25.
 */
...
export function baseTask(args: {
  ...
  return {
    ...
    hint2: args.hint2 ?? 'Вспомни правило.',
...
export function noAnswerLeak(text: string): boolean {
  return !/ответ\s*:|→\s*\S|правильн\w*\s*:/i.test(text);
}
